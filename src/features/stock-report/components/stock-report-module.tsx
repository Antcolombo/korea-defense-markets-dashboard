    ...
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {ticker !== 'SPY' && spyRows.length ? (
            <Button type="button" size="sm" variant={mode === 'indexed' && benchmarks.spy ? 'secondary' : 'outline'} className="h-8 px-2 font-mono text-xs" disabled={mode !== 'indexed'} onClick={() => setBenchmarks(current => ({ ...current, spy: !current.spy }))}>
              SPY
            </Button>
          ) : null}
