  ...
  for (const ticker of tickers) {
    try {
      const result = await prisma.$queryRawUnsafe<FundamentalRow[]>(
        `SELECT "ticker", "periodEnd", "fiscalPeriod", "currency", "revenue", "grossProfit", "operatingIncome", "ebitda", "netIncome", "epsDiluted", "freeCashFlow", "cash", "debt", "sharesDiluted", "dataStatus", "asOfDate" FROM "FundamentalSnapshot" WHERE "ticker" = $1${cutoff ? ' AND "asOfDate" <= $2' : ''} ORDER BY "asOfDate" DESC, "periodEnd" DESC LIMIT 1`,
  ...
  for (const ticker of tickers) {
    try {
      const result = await prisma.$queryRawUnsafe<EstimateRow[]>(
        `SELECT "ticker", "periodEnd", "fiscalPeriod", "estimateDate", "revenueEstimate", "epsEstimate", "ebitdaEstimate", "provider", "dataStatus", "asOfDate" FROM "EstimateSnapshot" WHERE "ticker" = $1${cutoff ? ' AND "asOfDate" <= $2' : ''} ORDER BY "asOfDate" DESC, "estimateDate" DESC LIMIT 1`,
        ...(cutoff ? [ticker, cutoff] : [ticker])
      )
      if (result[0]) rows.set(ticker, result[0])
    } catch {
